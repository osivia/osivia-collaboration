$JQry(function() {
	...
	// Sortable
	$JQry("ul.sets-edition-sortable").sortable({
		cancel: "a",
		connectWith : "ul.sets-edition-sortable",
		cursor : "move",
		...
		update : function(event, ui) {
			var $target = $JQry(event.target),
				...
				$ul = $item.closest("ul"),
				active = $ul.hasClass("active-sets");
		...
		stop : function(event, ui) {
			var $target = $JQry(event.target),
				...
				$activeTasks = $formGroup.find("ul.active-sets > li")
	...
	$JQry(".sets-edition-sortable").disableSelection();
	
	

	
	
    $JQry(".sets-management select.select2[name=documentWebId]").each(function(index, element) {
        var $element = $JQry(element),
        ...
        $element.on("select2:select", function(event) {
			var $target = $JQry(event.target),
				...
				$submit = $form.find("input[type=submit][name=addSets]");
    ...
    $JQry(".sets-management select.select2[name=filterType]").each(function(index, element) {
        var $element = $JQry(element),
            options = {
                theme : "bootstrap",
                width : "resolve"
